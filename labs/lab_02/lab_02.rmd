...
selected_md_voters2 <- md_voters_2024 |> filter(STATE_REGISTRATION_DATE == COUNTY_REGISTRATION_DATE) |> filter(STATE_REGISTRATION_DATE > "2024-07-21")

nrow(selected_md_voters2)
```
...
**A1**Total new registants are 17083 since July 21. Montgomery County has the highest number of new registrants - 4848. Compared to the link, even though Prince George's county has a higher population (2nd rank), it ranks fourth here. 

```{r}
selected_md_voters <- md_voters_2024 |> filter(STATE_REGISTRATION_DATE > "2024-07-21")

selected_md_voters|>group_by(COUNTY)|>summarise(count =n())|> arrange(desc(count))
nrow(selected_md_voters)
```
...
**A2** They are different. July 21 is week ~30 which sees high registration count (7000s) compared to the rest of the weeks. The other weeks are in the 2000-4000 range. However, week 24 sees the highest voter count at 7540. 
...
**A3** June. June 10th - June 16th, i.e, Week 24 has the highest number of registrations. From what I see online, the presidential debate was on June 27 so maybe that caused people to register. 

```{r}
md_voters_2024 |> mutate(week_number = week(COUNTY_REGISTRATION_DATE))|>
  group_by(week_number) |>
  summarise(count = n())|>
  arrange(desc(count))
```
...
**A4** (from highest to lowest) Montgomery County, Prince George's and Baltimore county have the most new voters. Democrats, Unaffiliated and Republicans have the highest new voters. 

```{r}
md_voters_2024  <- md_voters_2024 |> select(COUNTY)
md_voters_2024 <- md_voters_2024 |>
  mutate(
    county_category = case_when(
        COUNTY == "Garrett" | COUNTY == "Allegany" | COUNTY =="Washington" ~ "Western Maryland",
        COUNTY == "Frederick" | COUNTY =="Montgomery" | COUNTY =="Prince George's" ~ "Capital Maryland",
        COUNTY == "Anne Arundel" |COUNTY == "Baltimore City" | COUNTY =="Baltimore" |COUNTY == "Carroll" |COUNTY == "Harford" | COUNTY == "Howard" ~ "Central Maryland",
...
**A5** "Which areas have a lot of unaffiliated party voters" would be my question. This would be interesting data for campaigners on where to go to skew the vote in their favour. 
