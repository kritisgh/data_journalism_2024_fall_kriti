...
**Answer** 131,395 rows/observations and 24 columns/variables 
...
In the space below, write the data type for the "last_name" column and the "amount" columns (it's in light gray, offset by \< \>). Also offer your best guess as to what the "last_name" column and the "amount" column represent. **Answer** last name is 'character (chr)' data type, amount is 'double (dbl)' data type. "last_name" is likely the last names of donors and "amount" is the contribution made by each donor. 
...
**Answer** head() was simply returning the first 6 rows by default whereas the code above first groups by Date and counts the number of contributions (n) and stores it in the variable "count_contribs" made on each date
...
**Answer** Compared to the previous codeblock, this data is sorted in descending order. Therefore, now we can see on which date the highest number of contributions were made as the first row.
...
**Answer** In the previous codeblock, we were only grouping by date and counting the number of contributions made on each date but now we are grouping across 2 parameters: memo_text and the amount. This is followed by a summarize function which counts the number of contributions made in that denomination. So, the first row means 5915 contributions (n) worth $21.07 (amount) each were made towards the committee TRUMP NATIONAL COMMITTEE JFC, INC (memo_text)
...
**Answer** In the previous code block we were grouping two parameters: memo_text and amount. We did not sort the final data. Here, we are only grouping by memo_text but summarizing across two parameters: count (n) and summing the amount. In the previous code block, since we grouped by 'amount' first, it grouped all unique denominations. But here, since we sum the amount, it shows us the total contribution made toward a committee. Lastly, it is in descending order, so the first entry is the committee with the highest amount of contributions.
...
**Answer** This code block adds on top of the previous code block. It's almost the same except it also shows mean and median amount. This data now tells us the contribution made on average toward each committee. So, $41.49015896	was on average the contribution made toward TRUMP NATIONAL COMMITTEE JFC, INC. Whereas, the previous codeblock only revealed the total amount a committee received in donations, sorted highest to lowest. This codeblock also tells also gives us the median of each committee's donations, which might be more descriptive about the data, if the mean gets skewed by outliers. Lastly, here, the sorting is done highest to lowest on the number of contributions made and not the total donations received by each committee in the previous codeblock. 
I don't understand what " which of the top results from Maryland" means but I think Earmarked for HOGAN FOR MARYLAND INC.
### Task 11
...
Compared to the previous codeblock, what has changed and how did that affect the result? How is the data sorted? **Answer** This codeblock first groups by 'city'. So now the summarize functions calculates the total number of contributions made from each city, the sum total amount donated by each city, the mean of this sum total, the median of this sum total, and the minimum and maximum contribution made from each city. The previous codeblock grouped by memo_text, ie, the committees. This data reveals a city by city donation trend. Lastly, it sorts from highest to lowest based on the total amount of contributions so the first row denotes the city which donated the most amount of money. 
