...
1640 problems. The problem described is saying that the expected values for column 12 and 13 were boolean values such as 1/0 or true/false but instead the "actual" value is numeric. This is because guess_max was only run on first ten rows and it only saw N/A so it expected NA to be the case for all fields. 
...
no, no problems(). This is because all rows were checked in guess_max. 
...
**Task** Run the following codeblock to glimpse the data. What data type is the "district" field? What data type is the "precinct" field? What data type is the "mail" column? Answer below. **Answer** district is dbl (numeric) data, precinct is character (chr) and mail is dbl (numeric)
...
**Task** Run the following codeblock to group by mail votes, count the number of precinct results, then sort from highest to lowest on count. How many results are there where mail is NA? What's the implication there? Answer below. **Answer**: 402345 results are NA in mail. The implication is that it could be suggesting that 402345 votes were not mail in or there's no information available for those votes. But we also have 0 as a group so there is confusion between these two fields. Does 0 mean no mail in votes or does NA mean no mail in votes or that there was no information available? The data is not clear. 
...
**Task** Run the following codeblock to create a new column called "registration_month" that extracts the month and year from the "registr_dt" column. Group by the new "registration_month" column, count, and sort by "registration_month". How many registrations are there in the data for January 1900? What do you think the first 20 results suggests? Answer below. **Answer** 12 registrations in January 1900. Perhaps there were typos. Most computer systems have a default date so if nothing is filled in or a particular field is missing (eg: just year) it could have filled in January 1900 as a default date/year. 
The first 20 rows could suggest sparse record keeping of voter registration dates. I was curious about the spike in 1966 and found out about the Voting Rights Act in 1965. 
...
**Task** Run the following codeblock to find the number of registered voters grouped by voter status reason, using summarise() to count. Are any of these worthy of exploration to you? Why or why not? Answer below. **Answer** "deceased" is worthy of exploring because I wonder how they keep track of which voters are dead and to make sure their vote counts toward the election, and to remove their name from the voter roll after the election is over. I am also curious to find the overlaps in this date, where a voter could belong to more than one category. 
...
**Task** Run the following codeblock to glimpse the data. What data type is the "amount" field? Answer below. **Answer** character (chr) data type. 
...
**Answer** Suspicious outliers such as unusually large values or unusually small values, gaps in data such as time based gaps, such as we shouldnt see a birth year greater than 2004 because the minimum age to vote in NC is 18. We also see 13 registered voters who are 120 years old.
...
**Answer** it changes the column name from 1_linenumber to x1_linenumber because it's good practice to not have a column start with a number. 
...
**Task** Run the following codeblock to attempt to add up the amount of all contributions. What does the error say when you run this code? What do you think the error means? Answer below. **Answer** the error is: Error in `summarise()`:
ℹ In argument: `total_amount = sum(amount)`.
Caused by error in `sum()`:
! invalid 'type' (character) of argument 
The error is because it is trying to sum "amount" which is character data type and to sum, it would expect numeric data. 
...
**Task** Run the following codeblock to attempt to change the datatype for the amount field to a number. What is the new data type (three letter code) for amount? Answer below. **Answer** amount has changed from chr(character data) to dbl (numeric data)
...
**Task** Run the following codeblock to check for duplicate rows using get_dupes(). How many are there? What is the donor name? Answer below. **Answer** 2 duplicates, Derrik Hamilton.
...
**Task** Run the following codeblock to use str_sub() to convert the ZIP codes that have nine digits to five digits, standardizing the field. Look at the difference in the result - what changed? **Answer** The zip codes only have 5 digits now. It no longer has the hyphen and the numbers after that as seen in the previous codeblock. 
...
2 misspellings of Conowingo remain - 'conowing' and 'conowingoo'
...
**Task** Run the following codeblock to use case_when() to fix misspellings of Conowingo in the "city" field. How many mispellings of Conowingo remain after running this code? Answer below. **Answer** 1 misspelling remains 
...
**Task** Run the following codeblock to use case_when() to fix misspellings of Conowingo in the "city" field using both the exact match method and the str_detect() method. How many mispellings of Conowingo remain after running this code? Answer below. Add a description of what this code does to your reference notebook. **Answer** 0 mispellings.
