...
**Task** Export the CSV file to your pre_lab_06 folder and read it into R by running the following codeblock. What problems do you see with the data? **Answer** The data has now become the header. There are a lot of NA values. 
...
**Task** Get rid of rows we don't need by running the codeblock below. What problems remain? **Answer** The titles of the headers are not descriptive. Some columns still have NA values like X29, X31-34. Candidate column also has party information that needs to be extracted.

```{r}
benzie_2024 <- read_csv("tabula-Benzie Aug 6,2024 Official Precinct Results.csv", col_names = FALSE)

benzie_2024 <- benzie_2024 |>
  filter(!is.na(X2)) |>  #if x2 is not NA I want to keep it
  rename(candidate = X1)
...
**Task** Run the codeblock below. What changed, and what issues or problems remain? **Answer** There are still columns that do not have descriptive titles. Header titles are fixed for the first 9 columns. A new column 'Office' was added to include the office a candidate ran for.

```{r}
benzie_2024 <- benzie_2024 |>
  separate(candidate, c("candidate", "party"), sep=' - ') |> #separate the candidate column, provide a list of the new columns you want from it, so first column here is going to be candidate still and second column is going to be party. split the existing Candidate column into 2 new columns, candidate and party, sep is the delimiter
