...
```{r}
primary_24 <- read_csv('data/maryland_primary_2024.csv') #both republic and democratic primary elections
```
...
**Answer** select() displays the columns that are passed into it as parameters. mutate() adds a new column to the dataframe. it takes in existing variables in the dataframe as arguments. Now, there is a new column called percent_election_day that takes in existing variable (election day) and divides it by total number of votes. So, the new column gives us the fraction of how many votes were done on election date out of the total number of votes. 
...
**Answer** The only difference from the previous codeblock is that this is sorted in descending order of percent_election_date. Candidate with the most number of people voting on election day show up first in this dataframe. This dataframe is in descending order of greatest percentage of voter turnout on election date. 
Aside: This data reveals that as journalists it might not make sense to expect to talk to democrats on election day because most voter turnout on election day are republican voters.
...
**Answer** This data reveals that democrats have embraced early voting. Now that the data is sorted in ascending order of voter turnout on election date, we can see visually that we have to go through a lot of pagination before we see republicans in the data.
...
**Answer** str_to_lower() converts to lower case, str_to_sentence() converts to sentence case, where only the first letter of sentence is capitalized.
These can take upto two arguments: string and locale (language). String is the input character/variable that has to be changed and locale takes in the language. If nothing is passed, it defaults to English.
str_to_upper("i") will return I since it defaults to English
str_to_upper("i", "tr") will return "İ" as Turkish has been passed as Locale
...
**Answer** The mutate function creates a new category called amount_category. It checks the value in pre-existing variable 'amount' and checks it across some conditionals that are defined. If the amount is less than hundred, the value in new variable 'amount_category' will become string value "less than 100", if the value is greater or equal to 100 and less than 500 the value in amount_category is Between \$100 and \$499 and so on. 
...
**Answer** I got an error running this codeblock: Error in summarize(group_by(maryland_winred_categories, amount_category),  : 
  
ℹ In group 1: `amount_category = "$3,000 or more"`.
Caused by error:
! object 'amount' not found

I believe this is because the variable 'amount' no longer exists in the new created maryland_winred_categories. Including another codeblock here to sort the data according to get largest category in terms of dollar amount.This reveals "Less than $100" is the largest category in terms of dollar amount. 
...
**Answer** I actually don't understand why we had to use a new variable here to hold the result because select() would only show a new view of the raw data. It wouldn't actually change the raw data. It is important for readability, makes the data more palatable. 
...
**Answer** I think each date from expenditure_date is grouped into the first of every month. The first is because of the floor function applied to it. Thus, this makes it easier to aggregate and compare expenditure over time. Had we not floored the expenditure_date, it would get grouped by each date. This data now shows total expenditure for each month.
