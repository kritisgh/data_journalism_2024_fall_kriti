...
```{r}
p00 <- get_decennial(geography = "state", variables = "P001001", year = 2000) #get_decennial is a tidycensus function, geography is a level
...
md <- get_acs(geography = "county",
              variables = c(medincome = "B19013_001"), #variable for median income, moe is margin of error
              state = "MD",
              year = 2022)
...
Is the difference between Calvert and Montgomery significant? **Answer** No, their difference is not significant because their margins of error overlap meaning that we can't say for sure which county places second in household income.

Is the difference between Howard and everyone else significant? **Answer** Yes, Howard County is far from any other county because even with its margin of error it is far from other counties. Significantly far from the next county, Calvert. 
...
**Task** We need a 5- digit zip code (again), so run the following codeblock to create a populate a new column called `zip5`, then find the zip code with the largest total amount of contributions.  **Answer** 20815 has the largest total amount of contributions. 
...
**Answer** 21202 zip code has median income below 50k at $47848. 

```{r}
md_zipcodes <- get_acs(geography="zcta", variables = "B19013_001", state='MD', year=2019) #get zip code data from census by doing geography = zcta!
...
**Task** Arrange md_zips_with_acs by estimate showing the highest median household income first. What questions do you have about these top zip codes? **Answer** I am curious about the kind of jobs the people from these zip codes have and their demographic data. I also want to explore what we discussed in class today in terms of gentrification. I would also like to see the trend in their estimate income over the years, which counties went up and down the list and see what factors caused this growth. 
```{r}
