...
```{r}
create_credentials(GROQ_API_KEY = "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")
```
...
**Answer** It gets pretty ridiculous after the first couple responses. The latter responses are still synonymous with "spikes" and are "porcupine" related but it starts bringing up random words like "rose" and "pete"

```{r}
groq_response <- axolotr::ask(
  prompt = "Give me 20 names for a pet PORCUPINE",
...
**Answer** Yes, it got that perfectly. 
```{r}
...
**Answer** I would change the language from "portfolio" to something else. The job description does not say anything about Budget and Appropriations, Financial Services, and Tax which is the result we get from "portfolio" I don't exactly know what "portfolio" here means myself but I would be more specific about it and probably specify to only take words mentioned in the job description. 
...
**Answer** It does make sense. It gives the correct library and/or package to load. I initially got the suggestion to load the library Tidyverse but then got suggested to use the package MAgritte specifically in Tidyverse, so it changes up. 
...
**Answer** I don't see it making any assumptions. It does some standard clean up for the data in the beginning and then suggests running statistics on them across age, DOB, region, party affiliation, all of which is provided in the dummy data. It would definitely help me understand the data and I am glad it takes care of th busy work such as cleaning up the data and the obvious questions out of the way. I think once those correlations are made, we can look into anything interesting that might crop up and go from there. It is a good stepping stone. 
...
**Answer** I think it does pretty well. This type of feature would be great for ALT text and increase the accessibility of graphics on the web. I have written a lot of ALT text and I always wonder how granular I should get. From a word count point of view, I know Twitter only allows 1000 characters so I would add that to the prompt as a constraint. 

The image depicts a military parachutist in mid-air, with the title "USE OF MILITARY RANK, JOB TITLE, OR PHOTOGRAPHS IN UNIFORM DOES NOT IMPLY ENDORSEMENT BY THE DEPARTMENT OF DEFENSE" superimposed over the image. The image is intended to convey the message that the depiction of a military member in uniform does not imply endorsement or approval from the Department of Defense.

* A military parachutist in mid-air:
	+ The parachutist is wearing a tan uniform with a helmet, goggles, and a harness.
	+ He is in a prone position, with his arms extended and his legs bent at a 90-degree angle.
	+ His parachute is partially deployed, with the lines and canopy visible.
* A backpack with an American flag:
	+ The backpack is black with a red, white, and blue American flag attached to it.
	+ The flag is folded and secured to the backpack with straps.
	+ The backpack appears to be attached to the parachutist's harness.
* A desert landscape in the background:
	+ The landscape is dry and barren, with small hills and rocky outcroppings.
	+ The sky is clear and blue, with a few wispy clouds visible.
	+ The overall atmosphere is one of vastness and emptiness.

The image effectively conveys the message that the depiction of a military member in uniform does not imply endorsement or approval from the Department of Defense. The use of a desert landscape as the background adds to the sense of vastness and emptiness, emphasizing the isolation and solitude of the parachutist in mid-air.
...
**Answer** It just keeps on loading (?) Haven't gotten a response. Have waited for 30 mins. Restarted and ran the code again as well. The installation happened fine. got a "success" message when I ran it in the terminal. 
