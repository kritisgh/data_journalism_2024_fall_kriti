...
**Task** Run the following code in the gray-colored codeblock below to extract all tables in the HTML and save it as a dataframe. What do you notice about the columns? **Answer** It is the first table from the URL. The columns are all text -- they have commas in them. 
Both 'Candidate' and the 'party' are in the column name. The 25th row has the total and percentage in the same column. 
...
results <- results[[1]] #things start at 1 here not 0
...
**Task** Run the following code to extract the table we want, clean up the names and remove the total row. Look at the rename() function - what do you think the numbers 2 through 6 are referring to? **Answer** the number 2 through 6 are the column indices of Harris, Trump, Oliver, Stein and Kennedy. Columns start with index 1 here, so we are not counting "Jurisdiction" here. 
...
results <- results[[1]] |>
  clean_names() |>
  rename(harris = 2, trump = 3, oliver = 4, stein = 5, kennedy = 6, shannon = 7) |> #rename by position, rename 2nd column to be harris, rename 3rd column as trump and so on. new name comes before old name. 
  slice(-25) |> #removing the last row, slice only works on rows
  mutate(harris = as.numeric(gsub(",","", harris))) |> #change harris into a numeric column, gsub is global substitute. substitute a comma with nothing
  ...
  mutate(shannon = as.numeric(gsub(",","", shannon)))
...
**Task** Run the following code in the gray-colored codeblock below to find the <li> tags. What does the output suggest about how many <ul> tags there are in the HTML source? **Answer** I would guess there are 2 unordered lists because the output is a list of 2. The li tags are a subset of the ul tag so I would imagine there are 2 unordered lists in the whole site with 55 list items divided amongst them. 

```{r}
cmte_results |> html_elements('li') #isolating specific HTML tags. works with html.element and html.elements but element will only grab the first element and elements will get em all.
```
...
**Task** Run the following code in the gray-colored codeblock below to find the text inside <li> tags. Describe the output: **Answer** It is a list of all 55 list items from the website. It gets the text from inside the list tag. Because it extracts the text, it also outputs any white spaces and return characters.
...
The output has all li tags that have an <a> tag in it. The trim argument removed any whitespace, carriage returns that we saw in the previous output. We now have two columns, "name" and "url". Name has the the text part of the li tags without the whitespaces and carriage returns. Since we haven't specifically extracted for the names of the committees, it also includes the li tags for the menu, footer, etc which is not required. 'url' has the link that was within the href attribute of <a> tag. All the ones we don't need start with a forward slash. 

```{r}
committees <- cmte_results |> html_elements("li a") #calling html elements which includes li tags that have an a tag in them 

committees_with_urls <- tibble( #making a df that has a name column and a name column, trim = true exludes any white space, return character, carriage  returns
...
committees_with_urls <- committees_with_urls |> 
  ...
  filter(!str_detect(url, "^/")) #in the url find out if it begins with a slash, caret means "start with" beginning of the column/line and remove that
```
