...
**Task** Run the following code to load the data. In the space below, indicate how many rows and columns it has. **Answer** 24 rows and 18 columns. 
...
**Answer** We first create a new column using the mutate function. This new column is the difference in unaffiliated voters from 2024 to 2022. After arranging this new column in descending order, the head function takes in the top 10 counties, i.e., the ones that have the biggest change in unaffiliated voters from 2024 to 2022.
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer** geom_bar takes in what we have to show in the bar chart visualisation we want to create. On X axis we have the County name, our categorical variable, and on Y axis we have the numeric value for that county, i.e., the change from unaffiliated voters from 2022 to 2024. I tried changing "weight" to Y here but got an error but when I change geom_bar to geom_col, I can only use Y and not weight. I'd love to know how this works. 
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock? **Answer** The bars are now reordered from lowest to highest change in unaffiliated voters. 
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock? **Answer** Now we can see the names of the counties clearly because the axes are reversed. 
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock? **Answer** theme_minimal removed the gray background from the previous result. It looks cleaner and there is more contrast now. 
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last version of the chart? **Answer** The Economist theme is applied to the chart. Makes the background blue. 
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock? **Answer** We added a title called "Counties with Biggest Jumps in Unaffiliated Voters." We also added a legend for each axes, X = "county", and Y = "net change" We also added a caption, "source: Maryland State Board of Elections" All this is done using the "labs" function referring to "labels"
...
maryland_top_counties |>
  ...
  theme_economist() +
  labs(
    title="Counties with Biggest Jumps in Unaffiliated Voters", #dont tell me what it is, tell me what the news/headline is
    x = "net change",
    y = "county",
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer** The code groups and counts the total number of contributions made on each date. 
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer** It gives us a line chart, with the X axis having the date. The Y axis is the value for total contributions. It shows us the variations/trend in how the number of contributions changed compared to simply sorting the data in descending order. 
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer**  scale_x_date changes the nature of the X axis, so each tick on X axis represents a week instead of a month. This makes it easier for us to see the spikes and troughs. 
scale_x_date takes in the argument date_breaks which takes in the intervals in which the data needs to be shown. Here, it is once every week. date_labels takes in the formatting in which the labels should be. Here %b is for the short version for Month, i.e., April becomes Apr. %d shows the date of the month, so 2022-06-01 would become 01. Without this formatting, the data once again becomes very cluttered to read because the entire date is read in. 

```{r}
md_winred_by_date |>
  ggplot() + 
  geom_line(aes(x=date, y=total_contributions)) + 
  scale_x_date(date_breaks = "1 week", date_labels = "%b %d") #%b %d Month name, short version, Day of month 01-31
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer** Theme allows changing aspects of the look and feel of the text, so here it takes in the X label of the X axis and tilts it at a 45 degree angle to make it more readable. hjust justifies the text horizontally. However, I am a little confused because when I played around with the numbers the labels moved up and down - vertically. I played around with the theme arguments in this codeblock to try out what else is possible. 
...
md_winred_by_date |>
  ...
  theme(
    axis.text.x = element_text(angle = 45,  hjust=1, color = "red", face = "bold")
