...
**Task** Run the following code to load data. Describe what you see in the answer space below. What columns exist in this data? **Answer** Columns: Precinct ID, Precinct Name, Congress, Legislature, Council, School, Polling ID, Shape length, Shape area, and Shape.
I see precinct names in PGcounty and projection data under the variable 'Shape' for map making. 
...
**Task** Run the following code. Describe the output in the space below: what kind of information does it contain? **Answer** Same as above. It's the same kind of CSV files we have been working with except now we have a Shape column which describes how the place will be drawn on a map based on longitude and latitude arguments. 
...
**Task** Run the following code. Describe the output in the space below. **Answer** Collection of boundaries of precincts in Prince George's counties. It is without any labels like zip codes and it also lacks any layering such as the shape of Maryland underneath. There are also some precincts that are in white which are called "ghost precincts"
...
**Task** Run the following code. Describe the output in the space below. **Answer** tigris got installed
...
Every county, its respective fips code and geoID and some other identifying data about those counties. We also get a Geometry variable which gives up the shape of the map of that county. 
...
**Task** Run the following code. Describe the output in the space below. **Answer** Instead of getting all counties, now we only get Maryland counties. 
...
**Task** Run the following code. Describe the output in the space below. **Answer** This map of Maryland does not visualise the bay. It shows all the county boundaries within MD. 
...
**Task** Run the following code. Describe the output in the space below. **Answer** Precinct boundaries are slightly inside the county boundaries. Theres a double line around the southeast end. COUNTYFP was used to get PG county data only
```{r}
ggplot() +
  geom_sf(data=md_counties |> filter(COUNTYFP == "033")) + #base layer first
  geom_sf(data=pg_precincts) + #top layers after that
  theme_minimal()
```
...
**Task** Run the following code. Describe the output in the space below. **Answer** This gives us a dataframe which contains turnout according to mode of voting such as mail, absentee, early, etc. 
...
**Task** Run the following code. Describe the output in the space below. **Answer** Now turnout rate in every precinct is joined with geographical data from PG county. 
...
**Task** Run the following code. Describe the output in the space below. **Answer** This is a choropleth map that shows number of people eligible to vote in each precinct of PG county.
...
**Task** Run the following code. Describe the output in the space below. **Answer** dataframe of eligible voters in each county of maryland for each party. Montgomery county, PG county and Baltimore county have the top 3 spots respectively. 
...
**Task** Run the following code. Describe the output in the space below. **Answer** This dataframe gives an estimate of the population of each county. 
...
**Task** Run the following code. Describe the output in the space below. **Answer** Same as above but now it has Geometry as an argument so now we have data to draw a map. 
...
**Task** Run the following code. Describe the output in the space below. **Answer** md_county_population has all names of counties suffixed by the word "county" and voters_by_county has all names of counties in title case.
...
**Task** Run the following code. Describe the output in the space below. **Answer** We now have the same county names for the code to execute the join function on. Both are in title case without the suffixed "county"
...
**Task** Run the following code. Describe the output in the space below. **Answer** this joins maryland's party registration data with MD's population data and their respective geometry/shape data.
...
**Task** Run the following code. Describe the output in the space below. **Answer** Code rectifies St. Mary's and Baltimore County so that they match. 
...
**Task** Run the following code. Describe the output in the space below. **Answer** shows number of voters per 10k habitants in each county. 
...
**Task** Run the following code. Describe the output in the space below. **Answer** Choropleth map showing number of voters per 10K people in a county on a color gradient.
...
**Task** Run the following code. Describe the output in the space below. **Answer** Same map but in a "magma" color scheme making it easier to differentiate amongst the counties. The former map which had a gradient of blue made it hard to discern the differences. 
...
**Task** Run the following code. Describe the output in the space below. What regional patterns do you see? **Answer** I personally dont see a difference between this map and the map above, not sure what I am missing.
...
**Task** Run the following code. Describe the output in the space below. **Answer** mutate function adds a new column to account for unaffiliated voters per 10k people.
...
**Task** Run the following code. Describe the output in the space below. What regional patterns do you see, especially on the ends of the scale? **Answer** Frederick, Howard and Anne Arundel have highest amount of Unaffiliated voters. Not sure why theree's no color legend for yellow. Frederick is interesting because if I remember corrrectly, that's where the most split ticketing also happened. 
